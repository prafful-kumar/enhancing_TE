...
class FinetuneTester():
    ...
    def validate(self):
        ...
        return best_score, self.best_params['lr'], self.best_params['wd']

    def evaluate(self, best_params=None):
        _logger.info("Best params {}".format(best_params))
        # load pretrained model
        self.model = ResNetBackbone(self.model_name)
        self.model = self.model.to(self.device)
        
        self.finetuner = FinetuneModel(self.model, self.num_classes, best_params['steps'],
                                       self.metric, self.device, self.feature_dim)
        test_score = self.finetuner.tune(self.trainval_loader, self.test_loader, best_params['lr'], best_params['wd'])
...
class ResNetBackbone(nn.Module):
    def __init__(self, model_name):
        ...
        _logger.info("Model {} checkpoint loaded".format(model_name))

        
        self.model.train()
        _logger.info("num parameters: {}".format(sum(p.numel() for p in self.model.parameters())))

        # for last block fine tuning use below code.
...
def prepare_data(dset, data_dir, batch_size, image_size, normalisation, num_workers, data_augmentation, seed):
    ...
    train_loader, val_loader, trainval_loader = get_train_valid_loader(dset, data_dir, normalise_dict, batch_size, 
                    image_size, random_seed=seed, num_workers=num_workers,
...
if __name__ == "__main__":
    ...
    #steps_ft = [5000, 10000, 15000, 20000]
    steps_ft = [5000]
    best_params = {}
    score = 0
    tester = None
    for step in steps_ft:
        _logger.info("Training models for total {} steps".format(step))
        tester_step = FinetuneTester(args.model, train_loader, val_loader, trainval_loader, test_loader,
                            metric, args.device, num_classes, grid=grid, steps=step)
    
        # tune hyperparameters
        score_step, best_lr, best_wd = tester_step.validate()
        ...
        if score_step > score:
            ...
            best_params['wd'] = best_wd

    test_score = tester.evaluate(best_params=best_params)
